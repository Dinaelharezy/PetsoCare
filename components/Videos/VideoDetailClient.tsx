...
export default function VideoDetailClient({ video }: { video: Video }) {
  ...
  const buildEmbedUrl = (id: string) => {
    const params = new URLSearchParams({
      ...
      autoplay: '0',
      ...(subtitleLang
        ? { cc_load_policy: '1', cc_lang_pref: subtitleLang }
        : { cc_load_policy: '0' }),
    })
    ...
    <div className="video-page" style={{ direction: 'rtl', padding: '20px' }}>
      ...
      <div className="video-container" style={{ maxWidth: '1000px', margin: '0 auto' }}>
            ...
            {SUBTITLE_OPTIONS.map(opt => (
              <button
                ...
                style={{
                  ...
                  borderRadius: '20px',
                  border:      '2px solid',
                  borderColor:  subtitleLang === opt.value ? 'rgb(100,180,40)' : '#d1d5db',
                  background:   subtitleLang === opt.value ? 'rgb(173,241,120)' : '#fff',
                  color:        subtitleLang === opt.value ? '#1a3a00' : '#6b7280',
                  fontWeight:   subtitleLang === opt.value ? '700' : '500',
                  fontSize:     '13px',
                  cursor:       'pointer',
        ...
        <div
          ...
          style={{ aspectRatio: '16/9', background: '#000', borderRadius: '16px', overflow: 'hidden' }}
        >
          {youtubeId ? (
            // key forces iframe reload when subtitle lang changes
            <iframe
              key={`${youtubeId}-${subtitleLang}`}
            ...
            <div style={{
              ...
              <i className="bi bi-youtube" style={{ fontSize: '48px', color: 'rgb(173,241,120)' }} />
              <a
                href={video.url}
                target="_blank"
                rel="noopener noreferrer"
                style={{ color: 'rgb(173,241,120)' }}
              >
                Open On Youtube
              </a>
            </div>
