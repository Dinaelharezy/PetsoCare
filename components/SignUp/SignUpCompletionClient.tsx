...
import Link from 'next/link'
import {TOTAL_STEPS, PasswordStrength, RegisterForm, useCompletion } from './hooks/useCompletion'
...
const STEP_LABELS = ['Your Info', 'Security', 'Review']
...
function StepPersonalInfo({ formData, onChange }: StepOneProps) {
  return (
    <div className="d-flex flex-column gap-3">
      <div className="row g-3">
        <div className="col-md-6">
      ...
      <div>
        ...
        <IconInput icon={<i className="bi bi-envelope" />}>
          <input
      ...
      <div>
        ...
        <IconInput icon={<i className="bi bi-telephone" />}>
          <input
...
function StepPassword({
  ...
  onChange, onTogglePass, onToggleConfirm,
}: StepTwoProps) {
  return (
    <div className="d-flex flex-column gap-3">
      ...
      <div>
        ...
        <div className="input-group">
          ...
          <button
            ...
            onClick={onTogglePass} style={{ borderRadius: '0 10px 10px 0' }}
          >
            <i className={`bi ${showPass ? 'bi-eye-slash' : 'bi-eye'}`} />
          </button>
        </div>
      ...
      <div>
        ...
        <div className="input-group">
          <span className="input-group-text bg-white">
            <i className={`bi ${
              !formData.confirmPassword ? 'bi-lock text-muted' :
              passwordsMatch  ? 'bi-check-circle-fill text-success' :
                               'bi-x-circle-fill text-danger'
            }`} />
          </span>
          <input
            ...
            style={{
              ...
              borderLeft: 'none', borderRight: 'none',
              borderColor: formData.confirmPassword
                ? passwordsMatch ? '#28a745' : '#dc3545'
                : '#ddd',
            }}
          />
          <button
            type="button" className="btn btn-outline-secondary"
            onClick={onToggleConfirm} style={{ borderRadius: '0 10px 10px 0' }}
          >
            <i className={`bi ${showConfirm ? 'bi-eye-slash' : 'bi-eye'}`} />
          </button>
        </div>
...
const REVIEW_FIELDS = (formData: RegisterForm) => [
  { label: 'Full Name', value: `${formData.firstName} ${formData.lastName}`.trim() || '—', icon: 'bi-person' },
  { label: 'Email',     value: formData.email || '—',  icon: 'bi-envelope' },
  { label: 'Phone',     value: formData.phone || '—',  icon: 'bi-telephone' },
  { label: 'Password',  value: '••••••••',             icon: 'bi-lock' },
...
function StepReview({ formData }: StepThreeProps) {
  return (
    <div>
      <p className="text-muted mb-4" style={{ fontSize: '14px' }}>
        Review your info before creating your account.
      ...
      <div className="d-flex flex-column gap-2">
        {REVIEW_FIELDS(formData).map(({ label, value, icon }) => (
          <div
            key={label}
          ...
          >
            ...
            <span style={{ maxWidth: '60%', textAlign: 'right', wordBreak: 'break-all' }}>
              {value}
            </span>
          </div>
        ))}
      ...
      <div
        ...
        style={{
          ...
          border: '1px solid rgb(199,242,167)',
        }}
      >
        <p className="mb-0" style={{ fontSize: '13px', color: 'rgb(60,120,30)' }}>
          <i className="bi bi-envelope-check me-2" />
...
export default function SignUpCompletionClient() {
  const {
    step, saveStatus, errorMsg,
    showPass, showConfirm,
    formData, strength,
    passwordsMatch, passwordsMismatch,
    handleChange, handleNext, handleBack, handleRegister,
  ...
  const renderStep = () => {
    switch (step) {
      case 1:
        return <StepPersonalInfo formData={formData} onChange={handleChange} />
      case 2:
        return (
          <StepPassword
            formData={formData}
    ...
    >
      <div className="w-100" style={{ maxWidth: '520px', padding: '1rem' }}>
          ...
          <div className="card-body p-5">
            ...
            >
              {step === 1 ? (
                ...
                <button onClick={handleBack} className="btn btn-outline-secondary" style={{ borderRadius: '10px' }}>
                  <i className="bi bi-arrow-left me-1" />Back
                </button>
              )}
              ...
              {step < TOTAL_STEPS ? (
                <button
                  onClick={handleNext} className="btn fw-semibold"
                  style={{
        ...
        <p className="text-center text-muted mt-3" style={{ fontSize: '13px' }}>
          Already have an account?{' '}
          <Link
            href="/login" className="text-decoration-none fw-semibold"
            style={{ color: 'rgb(100,170,70)' }}
          >
            Sign in
          </Link>
        </p>
