...
//         detail: { clinicName: vet.name, patientName: getCurrentUserName(), date: `Day ${selectedDate}` }
//       }))
//     }
//   }
...
//         <p className="text-muted">Please select a day to schedule your appointment. Emergency appointments available upon request.</p>
//       </div>
        ...
        {weekDays.length > 0 ? (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(80px, 1fr))', gap: '10px', marginBottom: '20px' }}>
            {weekDays.map((item) => (
              ...
              >
                <span style={{ display: 'block', fontSize: '1rem', fontWeight: '600', color: selectedDate === item.day ? '#7CB342' : '#333' }}>
                  {item.label}
                </span>
              </div>
            ))}
          ...
          <p className="text-muted">No available days found.</p>
        )}
        <div className="text-center mt-4">
          <Button
            onClick={handleConfirmAppointment}
            disabled={!selectedDate}
            style={{ backgroundColor: '#7CB342', border: 'none', padding: '12px 40px', borderRadius: '10px', fontWeight: '500', fontSize: '1rem' }}
          >
            Confirm Appointment
          </Button>
        </div>
