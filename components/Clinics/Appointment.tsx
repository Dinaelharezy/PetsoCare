  ...
  if (step === 'done') {
    return (
      <Card style={styles.card} className="p-5 mb-4 text-center">
        ...
        <h4 style={{ fontWeight: '700', color: '#2d2d2d', marginBottom: '8px' }}>
  ...
  return (
    <>
      <div className="section-header mb-3">
        <h3 style={{ fontSize: '1.5rem', fontWeight: '700', color: '#1a1a1a' }}>
          Book an Appointment
        </h3>
            ...
            <>
                ...
                {availableDates.map((date) => {
                  ...
                  return (
                    <div key={date} style={styles.dateChip(active)} onClick={() => setSelectedDate(date)}>
                      <span style={{ fontSize: '0.72rem', color: active ? '#7CB342' : '#999', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                        {f.weekday}
                      </span>
                      <span style={{ fontSize: '1.5rem', fontWeight: '700', color: active ? '#7CB342' : '#1a1a1a', lineHeight: 1.1 }}>
                        {f.day}
                      </span>
                      <span style={{ fontSize: '0.72rem', color: active ? '#7CB342' : '#999' }}>
                        {f.month}
                      </span>
                    </div>
                  )
              ...
              {loadingTimes ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: '10px', marginBottom: '28px' }}>
                  {availableTimes.map((time) => (
                    <div key={time} style={styles.timeChip(selectedTime === time)} onClick={() => setSelectedTime(time)}>
                      {time}
                    </div>
                  ))}
                </div>
              ...
              <div className="text-end">
                <Button
                  ...
                  style={styles.confirmBtn(!selectedDate || !selectedTime)}
                >
                  Next →
                </Button>
              </div>
            ...
            <>
              <div style={{ backgroundColor: '#f8fdf4', borderRadius: '14px', padding: '16px 20px', marginBottom: '24px', display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
                <div>
                  ...
                  <div style={{ fontWeight: '700', color: '#7CB342' }}>
              ...
              <h6 style={{ fontWeight: '700', color: '#1a1a1a', marginBottom: '20px', fontSize: '0.95rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                👤 Your Details
              </h6>
              ...
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  style={{ borderRadius: '12px', border: '2px solid #e8e8e8', padding: '12px 16px', fontSize: '0.95rem' }}
                />
              </Form.Group>
              ...
              <div className="d-flex justify-content-between">
                <Button variant="link" style={{ color: '#999', textDecoration: 'none', padding: 0 }} onClick={() => setStep('datetime')}>
                  ← Back
                </Button>
                <Button
                  onClick={() => setStep('confirm')}
                  disabled={!customerName.trim() || !phone.trim()}
                  style={styles.confirmBtn(!customerName.trim() || !phone.trim())}
                >
                  Review →
                </Button>
              </div>
          ...
          {step === 'confirm' && (
            <>
              ...
              <div className="d-flex justify-content-between align-items-center">
                <Button variant="link" style={{ color: '#999', textDecoration: 'none', padding: 0 }} onClick={() => setStep('info')}>
                  ← Back
                </Button>
                <Button
                  onClick={handleConfirmAppointment}
                  disabled={booking}
                  style={styles.confirmBtn(booking)}
                >
                  {booking ? (
