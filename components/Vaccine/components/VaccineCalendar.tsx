...
export default function VaccineCalendar({ vaccines }: VaccineCalendarProps) {
  ...
  const hasVaccineOnDate = (day: number) =>
    vaccines.some(vaccine => {
      // startDate هو الـ field الصح
      if (!vaccine.startDate) return false
      const d = new Date(vaccine.startDate)
      return (
        d.getDate()     === day &&
  ...
  const renderCalendarDays = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++)
      days.push(
        <div key={day} className={`calendar-day ${hasVaccineOnDate(day) ? 'has-event' : ''}`}>
          {day}
        </div>
      )
    return days
