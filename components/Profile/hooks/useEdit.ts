...
//   const res = await fetch(apiUrl(`${process.env.NEXT_PUBLIC_API_URL}/user/profile`), { cache: 'no-store' })
...
export function useEdit() {
  const router = useRouter()
  const { update } = useSession()
  const { profileData, setProfile, clearProfile } = useProfileStore()
  ...
  const populateForm = (data: any) => {
    ...
    const img = data.imageUrl ?? data.image
    if (img) {
      setImagePreviewUrl(img.startsWith('http') ? img : `${process.env.NEXT_PUBLIC_API_URL}${img}`)
    } else {
      setImagePreviewUrl('/woman.png')
    }
  }
  ...
  useEffect(() => {
    if (profileData) { populateForm(profileData); return }

    // ✅ من غير ما تبعت للـ backend مباشرة
    fetch(apiUrl('auth/me'), { cache: 'no-store' })
      .then(res => res.json())
      .then(data => { populateForm(data); setProfile(data, data.id ?? data.email) })
      .catch(err => console.error('Failed to load profile:', err))
  }, [])

  // ✅ كل الـ refresh من الـ proxy
  const refreshAndSyncStore = async () => {
    const res  = await fetch(apiUrl('user/profile'), { cache: 'no-store' })
    const data = await res.json()
    populateForm(data)
    setProfile(data, data.id ?? data.email)
    return data
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      if (imageFile) {
        const formData = new FormData()
        formData.append('file', imageFile)
        const imgRes  = await fetch(apiUrl('user/upload-image'), { method: 'POST', body: formData })
        const text    = await imgRes.text()
        let imgData: any
        try { imgData = JSON.parse(text) } catch { imgData = { message: text } }
        if (!imgRes.ok) throw new Error(imgData?.error ?? imgData?.message ?? 'Image upload failed')
        const rawUrl = imgData?.imageUrl ?? imgData?.url ?? imgData?.image ?? imgData?.filePath
        if (rawUrl) newImageUrl = rawUrl.startsWith('http') ? rawUrl : `${process.env.NEXT_PUBLIC_API_URL}${rawUrl}`
      }
      ...
      const resolvedImage = newImageUrl ?? (() => {
        const img = freshData?.imageUrl ?? freshData?.image
        if (!img) return undefined
  ...
  const handleChangePhone = async () => {
    ...
    try {
      // ✅
      const res = await fetch(apiUrl('auth/update-phone'), {
        method:  'PUT',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ phone }),
      })
  ...
  const handleCancel = () => { setErrorMsg(''); setSuccessMsg(''); router.back() }

  return {
    firstName, setFirstName, lastName, setLastName,
    email, phone, setPhone, address, setAddress,
    dateOfBirth, setDateOfBirth, imageFile, imagePreviewUrl,
    emailNotifications, currentPassword, setCurrentPassword,
    newPassword, setNewPassword, confirmPassword, setConfirmPassword,
    newEmail, setNewEmail, saving, errorMsg, successMsg,
    handleChangePhone, handleImageChange, handleToggleNotifications,
    handleSaveAll, handleChangePassword, handleChangeEmail, handleCancel,
  }
}