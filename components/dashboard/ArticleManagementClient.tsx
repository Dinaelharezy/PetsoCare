...
//         TitleEn:     article.titleEn   ?? article.title,    // 👈 fallback
//         SummaryEn:   article.summaryEn ?? article.summary,  // 👈 fallback
//         ContentEn:   article.contentEn ?? article.content,  // 👈 fallback
...
//         headers: { 'ngrok-skip-browser-warning': 'true' },
//       })
//       if (!response.ok) { alert(`Failed to delete: ${response.status}`); return }
...
export default function ArticleManagementClient() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/dashboard/articles/${deletingId}`, {
        method: 'DELETE',
        headers: { 'ngrok-skip-browser-warning': 'true' },
      })
      if (!response.ok) { 
