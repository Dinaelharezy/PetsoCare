...
// // ── geocode — Port Said biased ────────────────────────────────────────────
// // ── AI geocoding for Egyptian locations ───────────────────────────────────
// async function aiGeocode(name: string, address: string, governorate: string): Promise<{ lat: number; lng: number } | null> {
//   const cacheKey = `${name}|${address}|${governorate}`.toLowerCase()
//   if (geocodeCache.has(cacheKey)) return geocodeCache.get(cacheKey)!

//   try {
//     const prompt = `You are a geocoding service. Given this Egyptian location, return ONLY a JSON object with lat and lng coordinates. No explanation, no markdown, just the JSON.
...
// Respond with exactly: {"lat": 00.0000, "lng": 00.0000}`
...
//       const coords = { lat: Number(parsed.lat), lng: Number(parsed.lng) }
//       geocodeCache.set(cacheKey, coords)
//       return coords
//     }
//   } catch (e) {
//     console.error('AI geocode error:', e)
//   }
//   return null
// }
...
//             })
//           } else if (c.address) {
//             const coords = await aiGeocode(c.name,c.address,c.governorate)
