...
//       //   for (const r of reports) {
//       //     // ✅ Priority 1: root-level lat/lng (Bite reports that used GPS)
...
//       //           symptoms:   r.details?.selectedSymptoms,
//       //         },
//       //       })
//       //       continue
//       //     }

//       //     // ✅ Priority 2: details.locationCity (DangerousAnimal / Bite with full address)
//       //     const locationCity: string | undefined = r.details?.locationCity
...
//       //     // ✅ Priority 3: fallback — geocode district + governorate
//       //     const addr = [r.district, r.governorate].filter(Boolean).join(', ')
//       //     if (addr) {
//       //       const coords = await geocode(addr)
//       //       if (coords) dynamic.push({
//       //         id: r.id, name: `${r.type ?? 'Report'} #${r.id}`, address: addr,
//       //         ...coords, type: 'report',
//       //         extra: {
//       //           status:     r.status,
//       //           reportType: r.type,
...
//     // ✅ الحالة 1: root-level lat/lng موجودة
//     const rootLat = Number(r.latitude ?? r.lat)
//     const rootLng = Number(r.longitude ?? r.lng)
//     if (rootLat && rootLng && !isNaN(rootLat) && !isNaN(rootLng)) {
//       lat = rootLat
//       lng = rootLng
//       addressForDisplay = [r.district, r.governorate].filter(Boolean).join(', ')
//     }

//     // ✅ الحالة 2: locationCity من details
//     const locationCity = r.details?.locationCity
//     if (locationCity && locationCity.trim()) {
//       // هل هي إحداثيات؟ (مثال: "31.27720, 32.26079")
...
//     // ✅ الحالة 3: لو لسه مفيش إحداثيات، جرب district + governorate
//     if (!lat || !lng) {
//       const addr = [r.district, r.governorate].filter(Boolean).join(', ')
//       if (addr) {
//         const coords = await geocode(addr)
...
//     // ✅ لو لقينا إحداثيات، نضيف الـ report
//     if (lat && lng && !isNaN(lat) && !isNaN(lng)) {
//       dynamic.push({
//         id: r.id,
//         name: `${r.type === 0 ? '🐕 Bite Report' : r.type === 1 ? '⚠️ Complaint Report' : '📋 Dangerous Animal'} #${r.id}`,
...
//           animalType: r.details?.animalType,
//           symptoms: r.details?.selectedSymptoms || r.details?.symptoms,
//           reporterName: r.name,
//           phone: r.phone,
...
//         // ── Parse symptoms string if needed ──────────────────────────
//         let symptomsDisplay = ''
...
//               symptomsDisplay = `🔍 ${parsed.join(', ')}<br/>`
//             }
//           } catch {}
//         }
...
export default function PetMap({ onSelectLocation, allowPinDrop = false }: Props) {
  ...
  useEffect(() => {
    ...
    // ── Reports ──────────────────────────────────────────────────────
try {
  // use getAllReports instead of getMapReports
  const raw = await getAllReports()
  const reports: any[] = Array.isArray(raw) ? raw : (raw?.data ?? [])
  ...
  for (const r of reports) {
    ...
    // ✅ CASE 2 & 3: Check locationCity (coordinates or address)
    if (!lat && locationCity && locationCity.trim()) {
      // Check if it's direct coordinates like "31.27720, 32.26079"
      const coordMatch = locationCity.match(/^(-?\d+(?:\.\d+)?),\s*(-?\d+(?:\.\d+)?)$/)
      if (coordMatch) {
        lat = parseFloat(coordMatch[1])
        lng = parseFloat(coordMatch[2])
        addressForDisplay = `📍 ${lat}, ${lng}`
        console.log(`✅ Report ${r.id} - direct coordinates from locationCity: "${locationCity}"`)
      } else if (locationCity.length > 5) {
        // It's an address string, try geocoding
        try {
          ...
          if (coords) {
            ...
            lng = coords.lng
            addressForDisplay = locationCity
            console.log(`✅ Report ${r.id} - geocoded successfully`)
          } else {
            console.log(`❌ Report ${r.id} - geocoding failed for: "${locationCity.substring(0, 50)}"`)
          }
        } catch (err) {
          console.error(`❌ Geocoding error for report ${r.id}:`, err)
        }
      }
    }

    // ✅ CASE 4: Fallback to district + governorate
    if (!lat) {
      const addr = [r.district, r.governorate].filter(Boolean).join(', ')
      if (addr && addr.length > 3) {
        try {
          ...
          console.error(`❌ Fallback error for report ${r.id}:`, err)
        }
      }
    }

    // Add report if we have coordinates
    if (lat && lng && !isNaN(lat) && !isNaN(lng)) {
