...
// // ── map report type → filter key ──────────────────────────────────────────
// const getReportFilterType = (type: number): 'report-bite' | 'report-animal' => {
//   if (type === 0) return 'report-bite'
//   return 'report-animal' // type === 1
// }
...
//           // ── skip rejected ──────────────────────────────────────────
//           if (r.status === 3) {
//             console.log(`🚫 Report ${r.id} is rejected, skipping`)
//             continue
//           }

//           // ── skip complaints (type=2) ───────────────────────────────
//           if (r.type === 2) continue

//           // ── debug: log all user-related fields for first report ────
//           if (dynamic.filter(l => l.type.startsWith('report')).length === 0) {
//             console.log(`🔍 Report fields sample (id=${r.id}):`, {
//               userId:     r.userId,
...
//             })
//             console.log(`🔍 currentUserId passed in:`, currentUserId)
//           }
...
//             // track my reports
//             if (currentUserId && (r.userId === currentUserId || r.reporterId === currentUserId)) {
//               myIds.add(r.id)
//             }
...
//         console.log(`✅ Total reports added to map: ${dynamic.filter(l => l.type.startsWith('report')).length}`)
...
//     // ── density calculation (grid cells ~0.02° ≈ 2km) ────────────────────
//     const GRID = 0.02
//     const densityMap = new Map<string, number>()
//     const reportLocs = visibleLocations.filter(l => l.type.startsWith('report'))
//     reportLocs.forEach(loc => {
//       const cellKey = `${Math.floor(loc.lat / GRID)}_${Math.floor(loc.lng / GRID)}`
//       densityMap.set(cellKey, (densityMap.get(cellKey) ?? 0) + 1)
//     })

//     const getDensityColor = (lat: number, lng: number): string => {
//       const cellKey = `${Math.floor(lat / GRID)}_${Math.floor(lng / GRID)}`
//       const count   = densityMap.get(cellKey) ?? 1
//       // absolute thresholds — not relative — so a single report stays green
//       if (count >= 7) return '#dc2626'   // 🔴 7+ كتير جداً
//       if (count >= 4) return '#f97316'   // 🟠 4-6 كتير
//       if (count >= 2) return '#facc15'   // 🟡 2-3 متوسط
//       return '#22c55e'                   // 🟢 1 قليل
//     }
//     // ──────────────────────────────────────────────────────────────────────
...
//       // reports → density color | clinics & my-reports → type color
//       const color =
//         isMyReport
...
//       // pulse ring for high-density reports (red zone)
//       const isHot    = loc.type.startsWith('report') && color === '#dc2626'
...
//         ? (() => {
//             const s = loc.extra.severity.toLowerCase()
//             const bg =
//               s === 'superficial' ? '#fef9c3' :
//               s === 'moderate' || s === 'deep' ? '#ffedd5' : '#fee2e2'
//             const fg =
//               s === 'superficial' ? '#854d0e' :
//               s === 'moderate' || s === 'deep' ? '#9a3412' : '#7f1d1d'
//             const label =
//               s === 'superficial' ? '🟡 Superficial' :
//               s === 'moderate'    ? '🟠 Moderate'    :
//               s === 'deep'        ? '🟠 Deep'        :
//               s === 'severe'      ? '🔴 Severe'      : `⚫ ${loc.extra.severity}`
//             return `<span style="display:inline-block;padding:2px 8px;border-radius:10px;background:${bg};color:${fg};font-size:11px;font-weight:700;margin-bottom:4px">${label}</span><br/>`
//           })()
//         : ''
...
//           }}>
//             <div style={{ fontWeight: 700, marginBottom: 4, color: '#374151', fontSize: 11 }}>
//               📊 Report Density
//             </div>
//             {[
...
//                   boxShadow: '0 1px 4px rgba(0,0,0,.25)', flexShrink: 0,
//                 }} />
...
//               const s  = selectedLoc.extra.severity.toLowerCase()
//               const bg = s === 'superficial' ? '#fef9c3' : s === 'deep' || s === 'moderate' ? '#ffedd5' : '#fee2e2'
//               const fg = s === 'superficial' ? '#854d0e' : s === 'deep' || s === 'moderate' ? '#9a3412' : '#7f1d1d'
//               const dotColor = s === 'superficial' ? '#facc15' : s === 'deep' || s === 'moderate' ? '#f97316' : '#dc2626'
//               const label = s === 'superficial' ? 'Superficial' : s === 'deep' ? 'Deep' : s === 'moderate' ? 'Moderate' : s === 'severe' ? 'Severe' : selectedLoc.extra.severity
//               return (
//                 <p className="mb-1 small d-flex align-items-center gap-2">
//                   <span>⚠️ Severity:</span>
//                   <span style={{ display:'inline-flex', alignItems:'center', gap:5,
//                     padding:'2px 10px', borderRadius:12, background:bg, color:fg, fontWeight:700, fontSize:11 }}>
